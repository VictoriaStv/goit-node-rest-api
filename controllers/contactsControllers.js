...
export const deleteContact = async (req, res, next) => {
  try {
    const ownerId = req.user.id;
    const contactId = req.params.id.trim();
    const deleted = await removeContact(ownerId, contactId);
...
export const createContact = async (req, res, next) => {
  try {
    const ownerId = req.user.id;
    const newContact = await addContact(ownerId, req.body);
...
export const updateContact = async (req, res, next) => {
  try {
    const ownerId = req.user.id;
    const contactId = req.params.id.trim();
    const updated = await updateServiceContact(ownerId, contactId, req.body);
...
export const updateFavorite = async (req, res, next) => {
  try {
    ...
    const updated = await updateStatusContact(ownerId, contactId, { favorite });
    if (!updated) throw HttpError(404, "Not found");
    res.status(200).json(updated);
  } catch (err) {
    next(err);
